...
- **Detection:** native meeting apps combine process audio signals with bounded start/stop policies. Quiet grace preserves a start candidate but requires fresh audio to complete confirmation; a native-app handoff passes the same start gate. Browser auto-recording requires a supported Google Meet document and explicit in-call controls, confirmed across consecutive observations. Calendar events and arbitrary browser output never prove a call. Meet controls currently require supported English Accessibility labels; unavailable/unsupported documents abstain, and manual recording remains available. A bound browser session ends on explicit leave or bounded loss of call evidence, independent of other browser audio. While the exact previously verified window is minimized, or still holds the call's tab (matched by room code or the title seen at verification) but its call controls cannot be read, observation is suspended without ending or trimming the recording; neither can authorize a new recording. A closed call tab, a readable page without call controls, or a quit browser is a confident end after its grace. Only a grace expiry with nothing readable is uncertain: it does not start the calendar repeat cooldown, and it releases a recording the user started instead of stopping it. A recording the user starts for the call the detector is tracking, or one already capturing that call when it is verified, joins that detector session so the call's end stops it; manual mode leaves every stop to the user. Lost observations log a content-free read issue (for example `pageTooLarge` or `accessibilityTimeout`). Calendar grace applies only to native sessions.
...
- **Remote speaker names:** LokalBot does not read Google Meet participant names or speaking labels. Remote voices are named in the rename sheet from calendar guests and typed hints; attendance alone never names a voice. The retired Meet observer's leftover files are deleted by the hourly speaker-evidence cleanup, while names already applied to transcripts remain.
- **Audio clock:** the Core Audio callback copies only its host timestamp and validity with the existing pooled-buffer handoff. The writer records anchors after successful file writes, so microphone and system timing cannot bridge missing buffers, recovery padding, or invalid timestamps.
