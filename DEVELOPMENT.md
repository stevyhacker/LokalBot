...
- **Speaker attribution:** unconfirmed microphone speech stays unidentified, including old persisted track/diarization defaults. User confirmations and supported voice-profile evidence are separate from track origin. Cross-track loudness correlation marks possible echo without relying on identical ASR text; speech is retained unless the narrower waveform-and-text proof authorizes duplicate removal. Summaries and action ownership use the same normalized provenance. Calendar guests remain suggestions for multi-speaker meetings.
...
- **Speaker names from visuals (experimental, off by default):** a recording-scoped observer selects the expected Chrome/Meet document at no more than 2 Hz and retains that window binding. If several windows show the same call, the focused matching window establishes the initial binding. Structured participant labels are preferred; persistent bottom-left tile names also qualify after on-device OCR corroboration, without requiring hover controls. These names are retained in the encrypted session header independently of speaking intervals and appear under "Seen in this meeting" in the rename sheet. Presence alone cannot name a voice. Automatic visual naming additionally requires a known self tile, unambiguous activity, and recorded-audio alignment. The ScreenCaptureKit/Vision activity fallback accepts the legacy blue border/equalizer or a blue/peach outline paired with non-resting audio bars in that participant's named People-row control. Captures exclude window shadows and restart their sizing after a resize. Unsupported indicators abstain; live speaking accuracy and indicator lag still require staged validation in the [implementation plan](Docs/meeting-speaker-visual-attribution-plan-2026-09-07.md).
- **Clock and matching:** the Core Audio callback copies only its host timestamp and validity with the existing pooled-buffer handoff. The writer records anchors after successful file writes. Visual intervals cannot bridge missing buffers, recovery padding, source/layout changes, stale frames, or invalid timestamps. The original diarizer timeline supplies speaker boundaries. Four independent clear turns, 15 seconds of support, 98% usable support, and no competing two-second turn qualify for the provisional automatic visual rule. These are experimental rules, not probability estimates or measured accuracy.
...
- **Optional local voice profiles:** **Remember speakers on this Mac** separately exposes the pinned FluidAudio 0.15.8 `embedding256` chunk vectors during the existing diarization pass. Each job's lightweight manager shares retained model objects; segmentation/clustering settings stay unchanged. Only explicit user confirmations can enroll at least three independent clean turns totaling 15 seconds. Profiles use bounded normalized vectors, exact model/preprocessing fingerprints, source contributions, revisions, and deletion tombstones. Unknown and confusable voices abstain; recognition thresholds require held-out calibration. A profile database revision check rejects results computed before a rename, deletion, or enrollment change.
