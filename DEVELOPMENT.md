...
- **Speaker names from visuals (experimental, off by default):** a recording-scoped observer locates the recording-bound Chrome/Meet document across browser windows at no more than 2 Hz. Structured, explicitly named participant containers are preferred; the bounded ScreenCaptureKit/Vision fallback requires a matching name, active border, and equalizer shape. Unsupported provider structures abstain. Actual Chrome/Meet layout support and indicator lag still require the staged remote-Mac validation in the [implementation plan](Docs/meeting-speaker-visual-attribution-plan-2026-09-07.md); synthetic adapter contracts are not compatibility proof.
