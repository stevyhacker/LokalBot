...
- **UI:** sidebar Today | Meetings | Timeline | Ask | Agent | Settings. Ask searches as you type and keeps the conversation column in place. Source, result-type, and screen filters live in one menu. Keyword and available semantic rankings blend automatically, respecting the semantic-indexing preference. Return opens the selected result with playback paused; **Command-Return** or **Ask about results** submits a question bounded to the displayed meetings and screen moments. With no results, Ask uses the chosen source/day scope.
...
- **Meeting summary:** a single Summary tab shows the recap, editable actions, decisions, and open questions, with full notes behind **Show full notes**. Find-in-page opens the full notes automatically when needed. Processing badges remain on library rows; failures expose a conditional retry banner.
...
- **Today:** the default landing page combines capture state, the current day digest, open meeting actions, upcoming-meeting preparation, and direct routes to Ask, Timeline, and Agent Mode. The same digest component used in Timeline includes a compact expandable Yesterday line for the previous workday. Calendar setup lives in Settings; only the next meeting appears here.
...
- **Writing** — test and tune Autocomplete (the Cotyping engine, exclusions, acceptance behavior) and Dictation (shortcut style, paste vs. clipboard).
