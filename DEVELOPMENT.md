...
- **Neural diarization:** Settings → Recording → **Separate voices by speaker** partitions microphone and system audio before ASR. Pyannote Community-1 remains the default (threshold 0.70, step ratio 0.15, min segment 0.3 s). **Nemotron 3 (Preview)** is an opt-in, post-recording alternative for up to eight speakers, using FluidAudio 0.17.1's full-precision `offline` preset. Overlapping voices remain unresolved. Its approximately 200 MB CoreML download is pinned by revision and per-file SHA-256 under `Models/NemotronDiarization/` in app support. Preparation is shared with recording-time prewarm; model failures surface for retry instead of checkpointing generic-speaker success. Partial transcript keys include the backend/weight identity. See the [public AMI benchmark](Benchmarks/NemotronDiarization/results/2026-09-23/REPORT.md) and [integration notes](Benchmarks/NemotronDiarization/INTEGRATION.md).
